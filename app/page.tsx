...
export default function BonomayaCompleteApp() {
  const [isLoginOpen, setIsLoginOpen] = useState(false);
  const [authMode, setAuthMode] = useState("login"); // 'login' অথবা 'signup'
  ...
  const handleAuthSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // বর্তমানে এটি শুধু একটি সাকসেস মেসেজ দেখাবে
    alert(authMode === "login" ? "লগইন সফল হয়েছে!" : "আপনার অ্যাকাউন্ট তৈরি হয়েছে!");
    ...
    <main className="min-h-screen bg-[#FFFDF5] font-sans selection:bg-[#FFB237]/40 text-[#1A1A1A]">
        ...
        <div className="fixed inset-0 z-[200] flex items-end md:items-center justify-center bg-black/60 backdrop-blur-sm p-4 transition-all">
          <div className="bg-white w-full max-w-md rounded-t-[40px] md:rounded-[40px] p-8 relative shadow-2xl animate-in slide-in-from-bottom duration-300 border-t-8 border-[#FFB237]">
            ...
            <form className="space-y-4" onSubmit={handleAuthSubmit}>
              ...
              <button className="w-full bg-[#854442] text-white py-5 rounded-2xl font-black uppercase tracking-[0.2em] shadow-xl shadow-[#854442]/20 active:scale-95 transition-all">
            ...
            <div className="mt-8 text-center border-t border-gray-100 pt-6">
              <p className="text-sm font-bold text-gray-500">
                ...
                <button 
                  ...
                  className="text-[#FFB237] font-black underline underline-offset-4"
                >
      ...
      <section className="px-5 pt-6 bg-white pb-4">
        <div className="relative group">
          <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[#FFB237] transition-colors" size={20} />
          <input 
            type="text" 
            placeholder="আজকে কী খেতে মন চাইছে?" 
            className="w-full bg-gray-50 border-2 border-gray-100 py-4 pl-14 pr-6 rounded-[25px] text-sm font-bold focus:outline-none focus:border-[#FFB237] focus:bg-white transition-all shadow-sm"
          />
        </div>
      </section>

      {/* --- হিরো ব্যানার --- */}
      <section className="px-5 py-6 bg-white rounded-b-[40px]">
        <div className="bg-[#854442] rounded-[45px] p-10 text-white relative overflow-hidden shadow-2xl border-b-8 border-[#FFB237]">
          <div className="relative z-10">
            <div className="flex items-center gap-2 mb-3 bg-white/10 w-fit px-3 py-1 rounded-full border border-white/10">
              <Zap size={14} className="text-[#FFB237] fill-[#FFB237]" />
              <span className="text-[10px] font-black uppercase tracking-widest text-[#FFB237]">অফিশিয়াল অফার</span>
            </div>
            <h2 className="text-4xl font-black mb-4 leading-tight italic tracking-tighter">
              বিলাসবহুল স্বাদ, <br /><span className="text-[#FFB237]">সরাসরি কিচেন থেকে।</span>
            </h2>
            <button className="bg-[#FFB237] text-black px-8 py-3.5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl active:scale-95 transition-all">অর্ডার শুরু করুন</button>
          </div>
          <div className="absolute right-[-20px] bottom-[-20px] text-[150px] opacity-10 rotate-12">🥯</div>
        </div>
      </section>
      ...
      <section className="px-5 mt-10">
        <div className="flex justify-between items-center mb-6 px-2">
            <h3 className="text-2xl font-black tracking-tighter italic text-[#854442]">ক্যাটাগরি</h3>
            <ChevronRight className="text-[#FFB237]" />
        </div>
        <div className="grid grid-cols-2 gap-4">
          {categories.map((cat, i) => (
            <div key={i} className={`${cat.color} p-6 rounded-[35px] border-2 border-[#DFC8A8]/30 shadow-sm flex flex-col gap-4 active:scale-95 transition-all cursor-pointer group hover:border-[#FFB237]`}>
              <div className="w-14 h-14 bg-white rounded-3xl flex items-center justify-center text-3xl shadow-md group-hover:scale-110 transition-transform">{cat.icon}</div>
              <div className="flex flex-col">
      ...
      <section className="px-5 py-16">
        <div className="bg-white rounded-[45px] p-10 text-center border-2 border-[#DFC8A8]/30 shadow-sm relative overflow-hidden">
            <Sparkles className="mx-auto mb-6 text-[#FFB237]" size={40} />
            <h3 className="text-3xl font-black mb-4 italic tracking-tighter text-[#854442]">বনমায়া লাইভ বেকারি</h3>
            ...
            <div className="flex gap-4">
                <div className="flex-1 bg-[#FFFDF5] p-4 rounded-3xl border border-[#FFB237]/20">
                    <p className="text-xl font-black text-[#854442]">১০০%</p>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">তাজা উপকরণ</p>
                </div>
                <div className="flex-1 bg-[#FFFDF5] p-4 rounded-3xl border border-[#FFB237]/20">
                    <p className="text-xl font-black text-[#854442]">লাইভ</p>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">বেকিং সুবিধা</p>
                </div>
            </div>
        </div>
      </section>
      ...
      <section className="px-5 pb-10">
        ...
        <div className="grid grid-cols-1 gap-4">
          <a href="tel:16910" className="bg-[#854442] p-6 rounded-[35px] flex items-center gap-6 text-white active:scale-95 transition-all shadow-xl border-b-4 border-[#FFB237]">
          ...
          <div className="bg-white p-6 rounded-[35px] flex items-center gap-6 border-2 border-[#DFC8A8]/30">
            <div className="w-14 h-14 bg-[#FFB237]/10 rounded-2xl flex items-center justify-center text-[#FFB237]">
              <Mail size={28} />
            </div>
            <div>
              <p className="text-[10px] font-black uppercase text-gray-400 tracking-widest">ইমেইল করুন</p>
              <p className="text-md font-black text-[#854442]">support@bonomaya.com</p>
            </div>
          </div>
        </div>
      </section>

      {/* --- অফিসিয়াল ইয়োলো ফুটার --- */}
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-16 mb-16">
            <div className="col-span-1 md:col-span-2">
              ...
              <p className="text-[#854442]/80 text-sm font-bold leading-relaxed max-w-xs mb-10 italic underline underline-offset-4 decoration-white/30">মিরপুর ২ এর প্রিমিয়াম লাইভ বেকারি। তাজা স্বাদ আর বিশুদ্ধতার অঙ্গীকার।</p>
            ...
            <div>
              ...
              <ul className="space-y-4 text-[#854442]/80 text-sm font-black italic">
                ...
                <li className="hover:text-white cursor-pointer transition">ক্যারিয়ার</li>
              </ul>
            </div>
          </div>
